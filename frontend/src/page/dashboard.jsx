...
import { ArchiveBoxIcon, AcademicCapIcon, BookOpenIcon, DocumentTextIcon, UserGroupIcon, ClockIcon } from "@heroicons/react/24/outline";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [arsipRes, kelasRes, modulRes] = await Promise.all([api.get("/arsip"), api.get("/kelas"), api.get("/modul")]);
        setStats({
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow duration-300">
            ...
            <div className="mt-4">
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div className="bg-blue-600 h-2 rounded-full" style={{ width: `${Math.min(stats.arsip * 2, 100)}%` }}></div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow duration-300">
            ...
            <div className="mt-4">
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div className="bg-green-600 h-2 rounded-full" style={{ width: `${Math.min(stats.kelas * 5, 100)}%` }}></div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow duration-300">
            ...
            <div className="mt-4">
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div className="bg-purple-600 h-2 rounded-full" style={{ width: `${Math.min(stats.modul * 4, 100)}%` }}></div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors duration-200">
              ...
              <div className="flex-1">
                <p className="font-medium text-gray-800">Surat Masuk Baru: No. 2024/BLK/001</p>
            ...
            <div className="flex items-start p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors duration-200">
              ...
              <div className="flex-1">
                <p className="font-medium text-gray-800">Peserta Baru: Kelas Pemrograman Dasar</p>
                <div className="flex items-center mt-1 text-sm text-gray-500">
                  <ClockIcon className="h-4 w-4 mr-1" />
            ...
            <div className="flex items-start p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors duration-200">
              ...
              <div className="flex-1">
                <p className="font-medium text-gray-800">Modul Baru: Pengenalan JavaScript</p>
                <div className="flex items-center mt-1 text-sm text-gray-500">
                  <ClockIcon className="h-4 w-4 mr-1" />
                  Ditambahkan 3 jam yang lalu
                </div>
              </div>
            </div>
          </div>
